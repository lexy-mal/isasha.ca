...
class CompetitionManager {
  ...
  async load() {
    try {
      const res = await fetch('dance_assets/competitions.json');
  ...
  getDataPath(filename) {
    if (!this.activeCompetition) return `dance_assets/${filename}`;
    return `dance_assets/${this.activeCompetition.id}/${filename}`;
  }
